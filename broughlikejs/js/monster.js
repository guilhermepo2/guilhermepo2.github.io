class Monster {
    ...
    /*
    - 5/16
        - Since drawSprite operates on a sprite index we normally pass in whole numbers representing 16 pixel sprites. 
        - However, we can instead work in individual pixels by using fractions. So 5/16 means 5 pixels within a 16 pixel sprite.
    - i%3 this resets to 0 every 3 pips.
    - Math.floor(i/3) this increases by one every 3 pips.
    */

   // #note: wtf are these weird numbers, needs a good UI system to take care of this!
    drawHp() {
        ...
        if(newTile.passable) {
            ...
            if(!newTile.monster) {
                ...
                if(this.isPlayer != newTile.monster.isPlayer) {
                    // #note just making class variables like this... on the fly? wtf?
...
class Eater extends Monster {
    ...
    doStuff() {
        let neighbors = this.tile.getAdjacentNeighbors().filter(t => !t.passable && isBounds(t.x, t.y));
